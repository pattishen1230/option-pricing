...
for file in files:
    ...
    for current_date, current_features_labels in data_dict.items():
        ...
        for i in range(current_features_labels.shape[0]):
            ...
            current_features_group = [current_features_labels[i, :-1]]
            for j in range(len(data)):
                ...
                if target_date in data_dict:
                    ...
                    if len(target_index) > 0:
                        current_features_group.append(target_features_labels[target_index[0], :-1])
    ...
    new_features = new_features.reshape(len(new_features), 18)
    new_labels = new_labels.reshape(len(new_labels), 1)
    data_list = np.concatenate((new_features, new_labels), axis=1)
    df =pd.DataFrame(data=data_list, columns=['spot price1', 'days to maturity1', 'strike1', 'moneyness1', 'volatility1', 'interest rate1',
                                       'spot price2', 'days to maturity2', 'strike2', 'moneyness2', 'volatility2', 'interest rate2',
                                       'spot price3', 'days to maturity3', 'strike3', 'moneyness3', 'volatility3', 'interest rate3',
                                       'call price'])

    # 将 DataFrame 保存为 CSV 文件
    df.to_csv(os.path.dirname(os.path.dirname(os.getcwd())) + '/data/v1.2/' + file, index=False)
