...
if __name__=="__main__":
    ...
    writer = SummaryWriter('./logs_lstm')
    ...
    for file in files:
        ...
        # define a MyLSTM
        lstm = MyLSTM(6, 64, 6, 1).to(device)
        optim = Adam(lstm.parameters(), lr=0.0001)
        ...
        for epoch in range(2000):
            ...
            if validaton_loss < best_val_loss:
                ...
                torch.save(lstm.state_dict(), './lstm_best_models_final/lstm_best_val_'+ file + '.pth')

            if epoch % 10 == 0:
                print(epoch, datetime.now(), running_loss, validaton_loss, best_epoch)

        # 测试最好的模型，算出RMSE和MAPE
        # 重新定义MyLSTM（用之前储存的val_total_loss最小的MyLSTM模型的参数）
        best_lstm = MyLSTM(6, 64, 6, 1).to(device)
        best_lstm.load_state_dict(torch.load('./lstm_best_models_final/lstm_best_val_'+ file + '.pth'))
        ...
        for data in test_loader:
            ...
            prediction = best_lstm(test_feature)
        ...
        print(file + "_LSTM模型的RMSE误差是：{}".format(test_RMSE_loss))
        print(file + "_LSTM模型的MAPE误差是：{}".format(test_MAPE_loss))
