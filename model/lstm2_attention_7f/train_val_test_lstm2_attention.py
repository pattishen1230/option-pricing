...
class Feeder(torch.utils.data.Dataset):
    def __init__(self, data_path, train_val_test='train'):
        ...
        features[:, 6] = 0
        features = features.reshape(features.shape[0], 3, 7)
        features = features[:, ::-1, :]
        labels = data.iloc[:, [21]]
...
if __name__=="__main__":
    ...
    writer = SummaryWriter('./logs_lstm2_attention')
    ...
    for file in files:
        ...
        # define a LSTM
        lstm2_atn = MyLSTM2_ATN(7, 64, 6, 1).to(device)
        # lstm2_atn.load_state_dict(torch.load('./lstm2_attention_best_models_final/lstm2_attention_best_val_'+ file + '.pth'))
        optim = Adam(lstm2_atn.parameters(), lr=0.001)
        ...
        best_epoch = 0
        total_epoch = int(2e8/len(train_feeder))
        print("Total epochs are  %d ..." % total_epoch)
        for epoch in range(total_epoch):
            ...
            for data in train_loader:
                ...
                prediction = lstm2_atn(train_feature)
            ...
            for data in val_loader:
                ...
                prediction = lstm2_atn(val_feature)
            ...
            # 把val_total_loss最小的MyLSTM模型保存起来
            if validaton_loss < best_val_loss:
                best_epoch = epoch
                best_val_loss = validaton_loss
                torch.save(lstm2_atn.state_dict(), './lstm2_attention_best_models_final/lstm2_attention_best_val_'+ file + '.pth')

            if epoch % 100 == 0:
                print(epoch, datetime.now(), running_loss, validaton_loss, best_val_loss, best_epoch)

        # 测试最好的模型，算出RMSE和MAPE
        # 重新定义MyLSTM（用之前储存的val_total_loss最小的MyLSTM3模型的参数）
        best_lstm2_atn = MyLSTM2_ATN(7, 64, 6, 1).to(device)
        best_lstm2_atn.load_state_dict(torch.load('./lstm2_attention_best_models_final/lstm2_attention_best_val_'+ file + '.pth'))
        ...
        for data in test_loader:
            ...
            prediction = best_lstm2_atn(test_feature)
        ...
        print(file + "_LSTM2_attention模型的RMSE误差是：{}".format(test_RMSE_loss))
        print(file + "_LSTM2_attention模型的MAPE误差是：{}".format(test_MAPE_loss))
